...
pub fn load_and_parse(path: &PathBuf) -> Result<LanguageFile, String> {
    let ext = file_loader::validate_extension(path)?;
    let content = file_loader::load_file(path)?;

    match ext {
        SYSML_EXT => {
            let file = crate::language::sysml::parser::parse_content(&content, path)?;
            Ok(LanguageFile::SysML(file))
        }
        KERML_EXT => {
            // TODO: Implement KerML parser - currently returns empty placeholder
            Ok(LanguageFile::KerML(KerMLFile::empty()))
        }
...
pub fn parse_content(content: &str, path: &Path) -> Result<LanguageFile, String> {
    let ext = file_loader::validate_extension(path)?;
...
pub fn parse_with_result(content: &str, path: &Path) -> ParseResult<LanguageFile> {
    ...
    match ext {
        SYSML_EXT => {
            let result = crate::language::sysml::parser::parse_with_result(content, path);
            match result.content {
                Some(file) => ParseResult::success(LanguageFile::SysML(file)),
                None => ParseResult::with_errors(result.errors),
            }
        }
        KERML_EXT => {
            // TODO: Implement KerML parser - currently returns empty placeholder
            ParseResult::success(LanguageFile::KerML(KerMLFile::empty()))
        }
        _ => {
