...
fn test_with_location_chains_properly() {
    ...
    assert_eq!(error.location.as_ref().unwrap().file, Some("test.sysml".to_string()));
}
...
fn test_with_location_with_file_only() {
    ...
    assert!(error.location.is_some());
    assert_eq!(error.location.as_ref().unwrap().file, Some("file.sysml".to_string()));
...
fn test_type_mismatch_message_format() {
    ...
    let expected = format!(
        "{SEMANTIC_TYPE_MISMATCH_MSG}: expected 'A', found 'B' in test"
    );
...
fn test_type_mismatch_kind_contains_all_fields() {
    ...
    if let SemanticErrorKind::TypeMismatch { expected, found, context } = &error.kind {
...
fn test_circular_dependency_with_empty_cycle() {
    ...
    assert!(matches!(error.kind, SemanticErrorKind::CircularDependency { .. }));
}
...
fn test_circular_dependency_with_qualified_names() {
    ...
    assert!(error.message.contains("Package::A -> Package::B -> Package::A"));
}
...
fn test_undefined_reference_with_valid_name() {
    ...
    assert!(matches!(error.kind, SemanticErrorKind::UndefinedReference { .. }));
}
...
fn test_display_format_order() {
    let error = SemanticError::undefined_reference("MyVar".to_string())
        .with_location(Location {
